...
if __name__ == '__main__':
    ...
    hopper_exp = Bvft_(device=device, parameter_list=parameter_list,
                       parameter_name_list=parameter_name_list, policy_training_parameter_map=policy_parameter_map,
                       method_name_list=["Bvft"], self_method_name="Bvft", batch_size=batch_size[0],
                       process_num= process_number[0],traj_sa_number=traj_sa_number,sa_evaluate_times = sa_evaluate_time,
                       **common_params,
                       )
    hopper_exp.generate_all_offline_data()
    # sys.exit()
    time = hopper_exp.run([0])
    hopper_exp.draw_figure_6L([0])
    time_list.append(time)
    # result_list.append([batch_size[i], process_number[j], time])
    result_list.append([traj_sa_number, time])
    save_as_txt("Time_result_traj_sa_number",result_list)

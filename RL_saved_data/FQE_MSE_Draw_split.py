...
def run_FQE_evaluation(device,FQE_learning_rate,FQE_hidden_layer,FQE_total_step,FQE_episode_step,num_interval):
    ...
    true_list = []
    prediction_list = []
    number_steps = int((FQE_total_step/FQE_episode_step)/num_interval)
        ...
        for i in range(FQE_number_epoch):
            ...
            if (se):
                if(is_key_in_dict(FQE_model_name,FQE_normal_dictionary)):
                    true_list[steps].append(policy_normal_dictionary[policy_file_name])
                    prediction_list[steps].append(FQE_normal_dictionary[FQE_model_name])
                    steps+=1
    NMSE_list = []
    standard_error_list = []
    for i in range(len(true_list)):
        NMSE,standard_error = normalized_mean_suqare_error(true_list[i],prediction_list[i])
        NMSE_list.append(NMSE)
        standard_error_list.append(standard_error)

    return NMSE_list, standard_error_list
...
def Draw_MSE_graph(FQE_total_step,FQE_episode_step,num_interval):
    ...
    name_list = [["FQE_1e-4_" + "[128,256]_" + str(FQE_total_step)],
              ["FQE_1e-4_" + "[128,1024]_" + str(FQE_total_step)],
              ["FQE_2e-5_" + "[128,256]_" + str(FQE_total_step)],
              ["FQE_2e-5_" + "[128,1024]_" + str(FQE_total_step)]]
    labels = []
    for i in range(len(FQE_1_MSE)):
        labels.append((i+1)*(FQE_episode_step)*num_interval)

    FQE_returned_folder = "FQE_returned_result"
    FQE_file_name_list = ["FQE_MSE_1","FQE_MSE_2","FQE_MSE_3","FQE_MSE_4"]
    colors = generate_unique_colors(len(FQE_1_MSE))

    print(FQE_1_MSE)
    print(FQE_1_SE)
    print(labels)
    draw_mse_graph(combinations=name_list[0], means=FQE_1_MSE, colors=colors, standard_errors=FQE_1_SE,
                   labels=labels, folder_path=FQE_returned_folder, filename="FQE_MSE_split_1")
    draw_mse_graph(combinations=name_list[1], means=FQE_2_MSE, colors=colors, standard_errors=FQE_2_SE,
                   labels=labels, folder_path=FQE_returned_folder, filename="FQE_MSE_split_2")
    draw_mse_graph(combinations=name_list[2], means=FQE_3_MSE, colors=colors, standard_errors=FQE_3_SE,
                   labels=labels, folder_path=FQE_returned_folder, filename="FQE_MSE_split_3")
    draw_mse_graph(combinations=name_list[3], means=FQE_4_MSE, colors=colors, standard_errors=FQE_4_SE,
                   labels=labels, folder_path=FQE_returned_folder, filename="FQE_MSE_split_4")
